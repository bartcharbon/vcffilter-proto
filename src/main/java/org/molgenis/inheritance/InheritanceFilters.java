...
public class InheritanceFilters {
  ...
  public static final String PARENT_HMZ_ALT = "parentHmzAlt";
  public static final String NON_PENETRANCE = "nonPenetrance";
  private final Pedigree pedigree;
  private final boolean nonPenetrance;
  Map<String, Filter> filters;

  public InheritanceFilters(Iterable<VcfRecord> records, Pedigree pedigree, String alleleIdx, boolean nonPenetrance) {
    ...
    filters.put(INHERITANCE_AD_AR_FILTER,new InfoFilter("FILTER_LABELS", SimpleOperator.CONTAINS_WORD, "AD/AR"));
    //FIXME autosomal R and D
    filters.put(
        INHERITANCE_XL_FILTER,new InfoFilter("FILTER_LABELS", SimpleOperator.CONTAINS_WORD, "XL"));
    filters.put(INHERITANCE_UNKNOWN_FILTER,new InfoFilter("FILTER_LABELS", SimpleOperator.CONTAINS_NONE, "AD,AR,XL,AD/AR"));
    ...
    filters.put(PARENT_HMZ_ALT, getParentHmzAltFilter(pedigree));
  ...
  private Filter getUnaffectedParentHmzAltFilter() {
    return vcfRecord -> {
      ...
      return new FilterResult(isHmz(vcfRecord,PedigreeUtils.getUnaffectedParent(pedigree).getPatientId(),"1"),vcfRecord);
    };
  }
  ...
  private Filter getFatherHasVariantFilter() {
    return vcfRecord -> {
      Pedigree father = PedigreeUtils.getFather(pedigree);
      return new FilterResult(!isHmz(vcfRecord, father.getPatientId(),"0"),vcfRecord);
    };
  }

  private Filter getFatherAffectedFilter() {
    return vcfRecord -> new FilterResult(PedigreeUtils.getFather(pedigree).getAffected() == Affected.TRUE, vcfRecord);
  }
  ...
  private Filter getParentHmzAltFilter(Pedigree pedigree) {
    return vcfRecord -> {
      Pedigree parent = PedigreeUtils.getSingleParentPedigree(pedigree);
      return new FilterResult(isHmz(vcfRecord, parent.getPatientId(), "1"),vcfRecord);
    };
  }
